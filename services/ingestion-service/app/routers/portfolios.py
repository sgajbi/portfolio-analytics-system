...
async def ingest_portfolios(
    ...
    logger.info(f"Received request to ingest portfolios.", num_portfolios=num_portfolios)
    try:
        # This method will be created in the next step
        await ingestion_service.publish_portfolios(request.portfolios)
        logger.info(f"Portfolios successfully queued.", num_portfolios=num_portfolios)
        return {
            "message": f"Successfully queued {num_portfolios} portfolios for processing."
        }
    except Exception as e:
        logger.error("Failed to publish bulk portfolios", error=str(e), exc_info=True)
        raise HTTPException(

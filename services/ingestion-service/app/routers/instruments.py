...
async def ingest_instruments(
    ...
    logger.info("Received request to ingest instruments.", num_instruments=num_instruments)
    try:
        await ingestion_service.publish_instruments(request.instruments)
        logger.info("Instruments successfully queued.", num_instruments=num_instruments)
        return {
            "message": f"Successfully queued {num_instruments} instruments for processing."
        }
    except ValidationError as e:
        logger.error("Validation error during instrument ingestion.", num_instruments=num_instruments, error=str(e), exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Invalid instrument data: {e.errors()}"
        )
    except Exception as e:
        logger.error("Failed to publish bulk instruments due to an unexpected error.", num_instruments=num_instruments, error=str(e), exc_info=True)
        raise HTTPException(

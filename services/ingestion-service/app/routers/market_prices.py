...
async def ingest_market_prices(
    ...
    try:
        ...
        logger.error("Validation error during market price ingestion.", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Invalid market price data: {e.errors()}"
        )
    except Exception as e:
        logger.error("Failed to publish bulk market prices due to an unexpected error.", exc_info=True)
        raise HTTPException(

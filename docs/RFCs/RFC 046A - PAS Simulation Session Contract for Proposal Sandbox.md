# RFC 046A - PAS Simulation Session Contract for Proposal Sandbox

## Problem Statement
PAS currently supports ingestion/query flows for actual portfolio data but lacks a first-class session model for iterative proposal simulation. UI/BFF cannot persist and recalculate tentative proposal changes safely and repeatedly.

## Root Cause
- No PAS API contract for simulation session lifecycle.
...
## Proposed Solution
Introduce PAS simulation session capabilities with explicit lifecycle and projection APIs.
...
- existing PAS valuation/FX references (where available)

## Architectural Impact
- Adds a product-layer simulation context in PAS without mutating committed transactions.
- Creates a clean handoff contract to PA and DPM for delta analytics and policy evaluation.
- Enables BFF to orchestrate iterative workflows safely with session/version controls.
...
- Increased API/state surface in PAS.
...
5. Add BFF consumer examples and operational diagnostics.

## Success Criteria
- UI/BFF can create a session, post iterative changes, and retrieve projected positions deterministically.

...
Cross-repo pyramid measurement currently reports PAS `collect_errors` because:

- `tests/unit` and `tests/integration` import `openpyxl` but it is not listed in `tests/requirements.txt`.
- `tests/e2e/test_5_day_workflow.py` uses `@pytest.mark.dependency` while PAS runs with `--strict-markers`, and the `dependency` marker is not declared in pytest config.
...
4. Re-run PPD pyramid measurement to confirm PAS collection errors are resolved.

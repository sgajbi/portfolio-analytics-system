...
def test_sort_by_date(sorter):
    t1 = Transaction(
        transaction_id="t1", transaction_date=datetime(2023, 1, 5), quantity=1,
        portfolio_id="P1", instrument_id="A", security_id="S1", transaction_type="BUY",
        settlement_date=datetime(2023, 1, 5), gross_transaction_amount=1, trade_currency="USD",
        # --- FIX: Add required field ---
        portfolio_base_currency="USD"
    )
    t2 = Transaction(
        transaction_id="t2", transaction_date=datetime(2023, 1, 1), quantity=1,
        portfolio_id="P1", instrument_id="A", security_id="S1", transaction_type="BUY",
        settlement_date=datetime(2023, 1, 1), gross_transaction_amount=1, trade_currency="USD",
        # --- FIX: Add required field ---
        portfolio_base_currency="USD"
    )

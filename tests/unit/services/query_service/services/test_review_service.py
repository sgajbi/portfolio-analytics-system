...
from src.services.query_service.app.dtos.summary_dto import (
    SummaryResponse as SummarySubResponse,
    ResponseScope,
    WealthSummary,
    PnlSummary,
    IncomeSummary,
...
def mock_dependencies():
    ...
    mock_summary_service.get_portfolio_summary.return_value = SummarySubResponse(
        ...
        pnlSummary=PnlSummary(
            net_new_money=1, realized_pnl=2, unrealized_pnl_change=3, total_pnl=5
        ),
        ...
        allocation=AllocationSummary(
            byAssetClass=[AllocationGroup(group="Equity", market_value=90000, weight=0.9)]
        ),
    )
    ...
    mock_transaction_service.get_transactions.return_value = PaginatedTransactionResponse(
        ...
        transactions=[
            TransactionRecord(
                transaction_id="T1",
                security_id="SEC_AAPL",
                transaction_date=date(2025, 8, 29),
    ...
    mock_net_perf = PerformanceResponse(
        scope={},
        summary={
    ...
    mock_gross_perf = PerformanceResponse(
        scope={},
        summary={
            "YTD": PerformanceResult(
                start_date=date(2025, 1, 1),
    ...
    with (
        patch(
            "src.services.query_service.app.services.review_service.PortfolioService",
            return_value=mock_portfolio_service,
        ),
        patch(
            "src.services.query_service.app.services.review_service.SummaryService",
            return_value=mock_summary_service,
        ),
        patch(
            "src.services.query_service.app.services.review_service.PerformanceService",
            return_value=mock_performance_service,
        ),
        patch(
            "src.services.query_service.app.services.review_service.RiskService",
            return_value=mock_risk_service,
        ),
        patch(
            "src.services.query_service.app.services.review_service.PositionService",
            return_value=mock_position_service,
        ),
        patch(
            "src.services.query_service.app.services.review_service.TransactionService",
            return_value=mock_transaction_service,
        ),
        patch(
            "src.services.query_service.app.services.review_service.InstrumentService",
        ...
        yield {
            ...
            "performance": mock_performance_service,
            "risk": mock_risk_service,
            "position": mock_position_service,
            "transaction": mock_transaction_service,

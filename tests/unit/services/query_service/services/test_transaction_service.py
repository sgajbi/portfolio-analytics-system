...
async def test_get_transactions_maps_cashflow_dto_correctly(mock_transaction_repo: AsyncMock):
    ...
    mock_db_transaction = Transaction(
        transaction_id="T_WITH_CASHFLOW", transaction_date=datetime(2025,1,10), transaction_type="DEPOSIT",
        instrument_id="CASH", security_id="CASH", quantity=1, price=1, gross_transaction_amount=1, currency="USD",
        # This is the critical part: attach a related cashflow object
        cashflow=Cashflow(
            amount=Decimal("5000"), currency="USD", classification="CASHFLOW_IN",
            timing="BOD", calculation_type="NET",
            is_position_flow=True, is_portfolio_flow=True # No 'level' field
        )
    )

...
def test_day3_ibm_buy(api_endpoints, db_engine):
    ...
    # Poll for the final state of Day 3 for AAPL (roll-forward)
    poll_db_until(
        db_engine=db_engine,
    ...
    # Poll for the final state of Day 3 for IBM (new position)
    poll_db_until(
        db_engine=db_engine,
        query="SELECT market_value FROM daily_position_snapshots WHERE portfolio_id = :pid AND security_id = :sid AND date = :date",
        params={"pid": PORTFOLIO_ID, "sid": IBM_ID, "date": DAY_3},
        validation_func=lambda result: result is not None and result.market_value == Decimal("45600.0000000000"),
        fail_message="Pipeline did not correctly value IBM snapshot for Day 3."
    )

...
class SummaryRepository:
    ...
    async def get_wealth_and_allocation_data(
        ...
        stmt = select(ranked_alias, instrument_alias).filter(
            ranked_snapshots_subq.c.rn == 1, ranked_alias.quantity > 0
        )

        results = await self.db.execute(stmt)
        data = results.all()
        logger.info(
            f"Found {len(data)} open positions for portfolio '{portfolio_id}' for summary as of {as_of_date}."
        )
        return data
    ...
    async def get_realized_pnl(
        ...
        """
        stmt = select(func.sum(Transaction.realized_gain_loss)).where(
    ...
    async def get_total_unrealized_pnl(self, portfolio_id: str, as_of_date: date) -> Decimal:
        ...
        stmt = select(func.sum(ranked_snapshots_subq.c.unrealized_gain_loss)).filter(
            ranked_snapshots_subq.c.rn == 1, ranked_snapshots_subq.c.quantity > 0
        )

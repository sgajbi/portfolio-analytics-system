...
class PerformanceRequestScope(BaseModel):
    ...
    as_of_date: date = Field(
        ...
        description="The reference date for calculations like MTD, YTD. Defaults to today.",
    )
    reporting_currency: Optional[str] = Field(
        None, description="ISO currency code for reporting. Defaults to portfolio's base currency."
    )
    net_or_gross: Literal["NET", "GROSS"] = Field(
        "NET", description="Specifies whether to calculate Net or Gross performance."
    )
...
class PeriodBase(BaseModel):
    ...
    name: Optional[str] = Field(
        None,
        description="A unique name for this period, used as a key in the response. If omitted, a default name will be generated.",
    )
    breakdown: Optional[Literal["DAILY", "WEEKLY", "MONTHLY", "QUARTERLY"]] = Field(
        None, description="Optional breakdown of results for this period."
    )
...
class YearPeriod(PeriodBase):
    ...
    type: Literal["YEAR"]
    year: int = Field(
        ..., gt=1900, lt=2100, description="The calendar year to calculate performance for."
    )
...
PerformanceRequestPeriod = Annotated[
    Union[ExplicitPeriod, YearPeriod, StandardPeriod], Field(discriminator="type")
]
...
class PerformanceRequestOptions(BaseModel):
    ...
    include_annualized: bool = Field(
        True, description="Whether to include annualized returns for periods over one year."
    )
    include_cumulative: bool = Field(
        True, description="Whether to include the total cumulative return for each period."
    )
    include_attributes: bool = Field(
        False,
        description="Whether to include financial attributes like market values and cashflows.",
    )
...
class PerformanceResult(BaseModel):
    ...
    cumulative_return: Optional[float] = Field(
        None, description="The total geometric return for the period as a percentage."
    )
    annualized_return: Optional[float] = Field(
        None, description="The annualized return as a percentage, if applicable."
    )
    daily_return: Optional[float] = Field(
        None, alias="dailyReturn", description="The geometric return for this day."
    )
    weekly_return: Optional[float] = Field(
        None, alias="weeklyReturn", description="The geometric return for this week."
    )
    monthly_return: Optional[float] = Field(
        None, alias="monthlyReturn", description="The geometric return for this month."
    )
    quarterly_return: Optional[float] = Field(
        None, alias="quarterlyReturn", description="The geometric return for this quarter."
    )
...
class PerformanceResponse(BaseModel):
    ...
    scope: PerformanceRequestScope
    summary: Dict[str, PerformanceResult] = Field(
        ..., description="A dictionary mapping the requested period name to its summary result."
    )
    breakdowns: Optional[Dict[str, PerformanceBreakdown]] = Field(
        None, description="A dictionary containing detailed breakdowns, if requested."
    )

...
class MWRRequestScope(BaseModel):
    ...
    as_of_date: date = Field(default_factory=date.today, description="The reference date for relative calculations like MTD, YTD. Defaults to today.")

class MWRExplicitPeriod(BaseModel):
    """Defines a custom period with explicit start and end dates."""
    type: Literal["EXPLICIT"]
    name: Optional[str] = Field(None, description="A unique name for this period, used as a key in the response.")
...
MWRRequestPeriod = Annotated[
    Union[MWRExplicitPeriod, MWRStandardPeriod],
...
class MWRResult(BaseModel):
    ...
    start_date: date
    end_date: date
    mwr: Optional[float] = Field(None, description="The solved Money-Weighted Rate of Return for the period.")
...
class MWRResponse(BaseModel):
    ...
    scope: MWRRequestScope
    summary: Dict[str, MWRResult] = Field(..., description="A dictionary mapping the requested period name to its summary result.")
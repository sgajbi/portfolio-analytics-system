...
class SummaryService:
    ...
    def _calculate_allocation_by_dimension(
        ...
        UNCLASSIFIED_ALLOCATION_MARKET_VALUE.labels(
            portfolio_id=portfolio_id, dimension=dimension.value
    ...
    async def get_portfolio_summary(
        ...
        tasks = {
            "positions": self.summary_repo.get_wealth_and_allocation_data(
                portfolio_id, request.as_of_date
            ),
            "cashflows": self.summary_repo.get_cashflows_for_period(
                portfolio_id, start_date, end_date
            ),
            "realized_pnl": self.summary_repo.get_realized_pnl(portfolio_id, start_date, end_date),
            "unrealized_pnl_start": self.summary_repo.get_total_unrealized_pnl(
                portfolio_id, start_date - timedelta(days=1)
            ),
            "unrealized_pnl_end": self.summary_repo.get_total_unrealized_pnl(
        ...
        wealth_summary, pnl_summary, income_summary, activity_summary, allocation_summary = (
            None,
            None,
            None,
            None,
        ...
        if SummarySection.WEALTH in request.sections:
            total_cash = sum(
                s.market_value or Decimal(0)
                for s, i in data["positions"]
                if i.product_type == "Cash"
            )
            wealth_summary = WealthSummary(total_market_value=total_mv, total_cash=total_cash)

        if (
            SummarySection.ALLOCATION in request.sections
            and request.allocation_dimensions
            and data["positions"]
        ):
            allocation_dict = {
        ...
        if any(
            s in request.sections
            for s in [SummarySection.PNL, SummarySection.INCOME, SummarySection.ACTIVITY]
        ):
            ...
            for cf in raw_cashflows:
                if cf.classification == "TRANSFER":
                    if cf.amount > 0:
                        totals["TRANSFER_IN"] += cf.amount
                    else:
                        totals["TRANSFER_OUT"] += cf.amount
            ...
            if SummarySection.ACTIVITY in request.sections:
                activity_summary = ActivitySummary(
                    total_deposits=totals["CASHFLOW_IN"],
                    total_withdrawals=totals["CASHFLOW_OUT"],
                    total_transfers_in=totals["TRANSFER_IN"],
                    total_transfers_out=totals["TRANSFER_OUT"],
                    total_fees=totals["EXPENSE"],
                )
            if SummarySection.INCOME in request.sections:
                income_summary = IncomeSummary(
                    total_dividends=totals["DIVIDEND_INCOME"],
                    total_interest=totals["INTEREST_INCOME"],
                )
